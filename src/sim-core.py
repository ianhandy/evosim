...
def _generate_terrain(seed_str):
    """
    Generate terrain by delegating to terrain_gen.py's 11-stage pipeline.

    Unpacks TerrainResult into the global numpy arrays (elevations, biomes,
    tile_flags) and sets designated_volcanoes for the volcanic eruption system.
    """
    ...
    result = _tg.generate(seed_val, GRID_SIZE)

    elevations[:] = result.elevations
    biomes[:]     = result.biomes

    # Rebuild tile_flags from scratch:
    #   bit 2 = volcanic summit (used by _volcanic_eruption + biome updates)
    tile_flags[:] = 0
    tile_flags[result.volcanic_mask] |= 2
    for pr, pc in result.peak_locations:
        tile_flags[pr, pc] |= 2

    designated_volcanoes = list(result.peak_locations)
