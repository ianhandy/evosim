...
function renderPreview() {
  ...
  // Preview approximation — fast FBM noise with plate-boundary uplift.
  // Not pixel-perfect match to Python tectonic sim, but gives a visual hint.
  // The real terrain is generated by Pyodide after "Begin Observation".
  const crust = new Float32Array(gs * gs);
  const WL = 0.20; // preview water level for biome coloring

  for (let r = 0; r < gs; r++) {
    for (let c = 0; c < gs; c++) {
      // Multi-scale FBM
      let e = fbm(r / gs * 4, c / gs * 4, seedVal) * 0.7
            + fbm(r / gs * 8, c / gs * 8, seedVal + 777) * 0.3;

      // Edge falloff — ocean at borders
      const border = gs * 0.08;
      const ed = Math.min(r, c, gs - 1 - r, gs - 1 - c);
      if (ed < border) e *= (ed / border) ** 1.5;

      crust[r * gs + c] = e;
    }
  }

  // Normalize and remap to get ~30% water
  let lo = Infinity, hi = -Infinity;
  for (let i = 0; i < gs * gs; i++) { if (crust[i] < lo) lo = crust[i]; if (crust[i] > hi) hi = crust[i]; }
  if (hi > lo) for (let i = 0; i < gs * gs; i++) crust[i] = (crust[i] - lo) / (hi - lo);

  const sorted = Float32Array.from(crust).sort();
  const thresh = sorted[Math.min(Math.floor(gs * gs * 0.30), gs * gs - 1)];
  for (let i = 0; i < gs * gs; i++) {
    if (crust[i] <= thresh) crust[i] = (crust[i] / Math.max(0.001, thresh)) * 0.14;
    else crust[i] = 0.15 + (crust[i] - thresh) / Math.max(0.001, 1 - thresh) * 0.85;
  }
